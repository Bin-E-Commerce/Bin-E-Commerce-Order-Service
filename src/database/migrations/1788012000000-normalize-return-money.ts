...
export class NormalizeReturnMoney1788012000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `ALTER TABLE "order_return_requests" ADD COLUMN IF NOT EXISTS "refund_item_amount" numeric(14,2) NOT NULL DEFAULT 0`,
        );
        await queryRunner.query(`
      WITH item_totals AS (
        SELECT
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `ALTER TABLE "order_return_requests" DROP COLUMN IF EXISTS "refund_item_amount"`,
        );
    }
}
