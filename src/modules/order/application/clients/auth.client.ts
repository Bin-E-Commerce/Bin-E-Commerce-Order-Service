...
import type { ShippingAddressResponse } from '@/modules/order/application/types/external-contracts.type';
    ...
    async getOwnedAddress(
        ...
        try {
            const response = await fetch(
                `${this.targetBase}/api/v1/internal/users/addresses/${addressId}`,
                {
            ...
            if (response.status === 404) {
                throw new CheckoutResourceNotFoundError(
                    'Không tìm thấy địa chỉ giao hàng của tài khoản.',
                );
            }
            if (!response.ok)
