...
export class ShippingClient {
    ...
    async calculateQuote(
        input: OrderShippingQuoteInput,
    ): Promise<ShippingQuote> {
        let response: Response;
        try {
            response = await fetch(
                `${this.targetBase}/api/v1/internal/shipments/quotes`,
                {
                    method: 'POST',
                    headers: {
        ...
        const payload = (await response
            .json()
            .catch(() => ({}))) as ShippingQuote & { message?: string };
        if (!response.ok) {
            if (response.status === 400)
                throw new BadRequestException(
                    payload.message ??
                        'Địa chỉ giao hàng chưa đủ thông tin cho GHN.',
                );
            throw new BadGatewayException(
                payload.message ?? 'Không thể tính phí giao hàng lúc này.',
            );
        }
        if (!payload.deliverySupported)
            throw new BadGatewayException(
                'GHN chưa hỗ trợ giao tới địa chỉ này.',
            );
        return payload;
    }
}
