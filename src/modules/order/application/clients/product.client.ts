...
import type {
    ...
    ReservationLine,
} from '@/modules/order/application/types/external-contracts.type';
        ...
        try {
            ...
            if (response.status === 404) {
                throw new CheckoutResourceNotFoundError(
                    data.message ?? 'Không tìm thấy sản phẩm hoặc variant.',
                );
            }
            if (response.status === 409 || response.status === 422) {
                throw new CheckoutRejectedError(
                    data.message ??
                        'Sản phẩm không còn đủ điều kiện để đặt hàng.',
                );
            }
            if (!response.ok)
