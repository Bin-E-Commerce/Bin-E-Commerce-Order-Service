...
} from '@/modules/order/application/clients/shipping.client';
import {
    EmptyCartError,
...
import type { CreateCodOrderDto } from '@/modules/order/presentation/dto/create-cod-order.dto';
import type {
    OrderListResponse,
    OrderResponse,
    SellerOrderListResponse,
    SellerOrderResponse,
} from '@/modules/order/application/types/order-response.type';
import type { CheckoutQuoteItem } from '@/modules/order/application/types/external-contracts.type';
import type { OrderListQueryDto } from '@/modules/order/presentation/dto/order-list-query.dto';
import type { CancelOrderDto } from '@/modules/order/presentation/dto/cancel-order.dto';
import type { SellerOrderListQueryDto } from '@/modules/order/presentation/dto/seller-order-list-query.dto';
import type { SellerOrderUserContext } from '@/modules/order/application/types/seller-order-user-context.type';
import {
    fromCents,
    toCents,
} from '@/modules/order/application/utils/order-money.util';
...
export class OrderCommandService {
        ...
        try {
            ...
            if (this.isUniqueViolation(error)) {
                const concurrentOrder =
                    await this.orderRepository.findByIdempotency(
                        ownerId,
                        idempotencyKey,
                    );
                if (
    ...
    async getOwnedOrder(
        ...
        const order = await this.orderRepository.findOwnedById(
            ownerId,
            orderId,
        );
    ...
    async getShippingContext(
        ...
        if (!shopId?.trim())
            throw new BadRequestException('Thiếu shop context.');
        const order = await this.orderRepository.findSellerById(
            shopId,
            orderId,
        );
        if (!order)
    ...
    async cancelBySeller(
        ...
        if (!normalizedReason) {
            throw new BadRequestException(
                'Seller phải nhập lý do hủy đơn hàng.',
            );
        }
        const order = await this.orderRepository.findSellerById(
            shopId,
    ...
    getSoldQuantities(sellerOwnerId: string, productIds: string[]) {
        return this.orderRepository.findSoldQuantities(
            sellerOwnerId,
            productIds,
        );
    }
    ...
    async cancelOwnedOrder(
        ...
        const didCancel = await this.dataSource.transaction(async (manager) => {
            ...
            if (
                lockedOrder.fulfillmentStatus !== OrderFulfillmentStatus.TO_SHIP
            ) {
                throw new OrderCancellationConflictError(lockedOrder.status);
            }
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            const order = repository.create({
                ...
                totalAmount: fromCents(
                    toCents(subtotal) + toCents(input.shippingFee),
                ),
            ...
            const orderItems = input.items.map((item) =>
                itemRepository.create({
                    ...
                    quantity: item.quantity,
                    lineTotal: fromCents(
                        toCents(item.unitPrice) * BigInt(item.quantity),
                    ),
                    packageWeightGrams: item.packageWeightGrams,
        ...
        const quotes = await Promise.all(
            [...groups.entries()].map(async ([shopId, shopItems]) => {
                const weight = shopItems.reduce(
                    (sum, item) =>
                        sum + item.packageWeightGrams * item.quantity,
                    0,
                );
                if (
                    !shopItems.every(
                ...
                const width = Math.max(
                    ...shopItems.map((item) => item.packageWidthCm),
                );
