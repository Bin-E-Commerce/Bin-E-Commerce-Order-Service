...
describe('OrderDeliveryConfirmationService', () => {
    ...
    const mockTransaction = (order: Order | null) => {
        const orderRepository = {
            createQueryBuilder: jest
                .fn()
                .mockReturnValue(createQueryBuilder(order)),
    ...
    beforeEach(async () => {
        ...
        target = module.get<OrderDeliveryConfirmationService>(
            OrderDeliveryConfirmationService,
        );
    });
    ...
    it('should mark an order as delivered and open confirmation window', async () => {
        ...
        expect(order.deliveryConfirmationDeadline).toEqual(
            new Date('2026-09-03T10:00:00.000Z'),
        );
        expect(
            mockOrderEvents.publishDeliveryAwaitingConfirmation,
    ...
    it('should sync shipment stages monotonically', async () => {
        ...
        // Act
        await target.syncShipmentStatus(
            orderId,
            'IN_TRANSIT',
    ...
    it('should complete an order when customer confirms receipt', async () => {
        ...
        const dto: DeliveryConfirmationDto = {
            decision: DeliveryConfirmationDecision.RECEIVED,
        };
        ...
        expect(order.deliveryConfirmationStatus).toBe(
            OrderDeliveryConfirmationStatus.CONFIRMED,
        );
        expect(mockOrderEvents.publishDeliveryConfirmed).toHaveBeenCalledWith(
            orderId,
        );
    });
    ...
    it('should move an order to return/refund when customer reports a delivery issue', async () => {
        ...
        expect(order.fulfillmentStatus).toBe(
        ...
        expect(
            mockOrderEvents.publishDeliveryIssueReported,
        ).toHaveBeenCalledWith(orderId);
    });
    ...
    it('should skip auto-complete before the confirmation deadline', async () => {
        ...
        expect(order.fulfillmentStatus).toBe(OrderFulfillmentStatus.DELIVERED);
        expect(
            mockOrderEvents.publishDeliveryAutoConfirmed,
