...
@Injectable()
export class OrderDeliveryAutomationService
    implements OnModuleInit, OnModuleDestroy
{
    ...
    private async processExpiredOrders(): Promise<void> {
        try {
            const orders =
                await this.orderRepository.findExpiredDeliveryConfirmations(
                    new Date(),
                );
            await Promise.all(
                orders.map((order) =>
                    this.deliveryConfirmation.autoComplete(order.id),
                ),
            );
        } catch (error) {
            this.logger.error(
                `Không thể auto-complete order hết hạn: ${String(error)}`,
            );
        }
    }
}
