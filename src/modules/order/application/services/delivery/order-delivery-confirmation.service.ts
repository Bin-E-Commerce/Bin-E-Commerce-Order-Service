...
import type { ShipmentStatus } from '@common/kafka/events/shipping.events';
import {
    DeliveryConfirmationDecision,
    DeliveryConfirmationDto,
} from '@/modules/order/presentation/dto/delivery-confirmation.dto';
    ...
    async markDelivered(orderId: string, occurredAt: string): Promise<void> {
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            if (
                ...
                order.deliveryConfirmationStatus ===
                    OrderDeliveryConfirmationStatus.ISSUE_REPORTED
            ) {
                return;
            }
            ...
            order.deliveryConfirmationStatus =
                OrderDeliveryConfirmationStatus.PENDING;
            order.deliveryConfirmationMethod = null;
            order.deliveryConfirmationDeadline = new Date(
                deliveredAt.getTime() + AUTO_CONFIRMATION_DELAY_MS,
            );
        ...
        if (changed)
            await this.orderEvents.publishDeliveryAwaitingConfirmation(orderId);
    }
    ...
    async confirm(
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            if (
                ...
                (order.fulfillmentStatus === OrderFulfillmentStatus.COMPLETED ||
                    order.deliveryConfirmationStatus ===
                        OrderDeliveryConfirmationStatus.AUTO_CONFIRMED)
            ) {
                return;
            }
            if (
            ...
            if (order.fulfillmentStatus !== OrderFulfillmentStatus.DELIVERED) {
                throw new ConflictException(
                    'Đơn hàng chưa ở trạng thái chờ xác nhận.',
                );
            }
            ...
            if (dto.decision === DeliveryConfirmationDecision.RECEIVED) {
                ...
                order.deliveryConfirmationStatus =
                    OrderDeliveryConfirmationStatus.CONFIRMED;
                order.deliveryConfirmationMethod =
                    OrderDeliveryConfirmationMethod.CUSTOMER;
                order.deliveryConfirmedAt = new Date();
                order.completedAt = order.deliveryConfirmedAt;
                await manager.getRepository(Order).save(order);
                await this.orderEvents.enqueuePurchaseCompleted?.(
                    orderId,
                    manager,
                );
            ...
            if (!dto.reason)
                throw new BadRequestException(
                    'Vui lòng chọn lý do đơn hàng có vấn đề.',
                );
            ...
            if (itemIds.some((itemId) => !orderItemIds.has(itemId))) {
                throw new BadRequestException(
                    'Sản phẩm báo lỗi không thuộc đơn hàng.',
                );
            }
        ...
        if (eventStatus === 'issue')
            await this.orderEvents.publishDeliveryIssueReported(orderId);
    ...
    async autoComplete(orderId: string): Promise<boolean> {
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            order.deliveryConfirmationStatus =
                OrderDeliveryConfirmationStatus.AUTO_CONFIRMED;
            order.deliveryConfirmationMethod =
                OrderDeliveryConfirmationMethod.AUTO;
            order.deliveryConfirmedAt = new Date();
            order.completedAt = order.deliveryConfirmedAt;
            await manager.getRepository(Order).save(order);
