...
import { ShippingClient, type GhnAddressSelectionInput } from "../../clients/shipping.client";
import type { CreateOrderReturnDto, InspectOrderReturnDto, ReviewOrderReturnDto } from "../../dto/order-return.dto";
...
export class OrderReturnService {
  constructor(
    @InjectRepository(OrderReturnRequest) private readonly repository: Repository<OrderReturnRequest>,
  ...
  async create(ownerId: string, orderId: string, dto: CreateOrderReturnDto) {
    ...
    if (![OrderFulfillmentStatus.DELIVERED, OrderFulfillmentStatus.COMPLETED].includes(order.fulfillmentStatus)) throw new ConflictException("Đơn hàng chưa đủ điều kiện hoàn hàng.");
    if (order.returnWindowUntil && order.returnWindowUntil < new Date()) throw new ConflictException("Đã hết thời hạn hoàn hàng.");
    this.validateEvidence(dto);
    const itemMap = new Map((order.items ?? []).map((item) => [item.id, item]));
    const itemIds = [...new Set(dto.itemIds)];
    if (itemIds.some((itemId) => !itemMap.has(itemId))) throw new BadRequestException("Sản phẩm hoàn không thuộc đơn hàng.");
    const shopIds = new Set(itemIds.map((itemId) => itemMap.get(itemId)?.sellerShopId).filter(Boolean));
    if (shopIds.size !== 1) throw new BadRequestException("Mỗi yêu cầu chỉ được thuộc một shop.");
    const shopId = [...shopIds][0] as string;
    const existing = await this.repository.findOne({ where: { orderId, ownerId, shopId, status: OrderReturnStatus.REQUESTED } });
    if (existing) return existing;
    const itemRefundAmount = this.roundVnd(itemIds.reduce((sum, itemId) => sum + toCents(itemMap.get(itemId)!.lineTotal), 0n));
    const shippingRefundAmount = this.calculateShippingRefund(order, itemIds, itemRefundAmount, dto.reason, shopId);
    ...
    const returnShippingCost = await this.quoteReturnShippingCost(order, itemIds, itemRefundAmount, shopId);
    const returnShippingFee = this.isSellerFault(dto.reason) ? 0n : returnShippingCost;
    const totalRefundAmount = this.capRefundAmount(
      itemRefundAmount + shippingRefundAmount - returnShippingFee,
      order.totalAmount,
    );
    const request = this.repository.create({
      orderId, ownerId, shopId, sellerUserId: itemMap.get(itemIds[0]!)?.sellerOwnerId ?? null,
      itemIds, status: OrderReturnStatus.REQUESTED, reason: dto.reason,
      description: dto.description?.trim() || null, reviewNote: null, evidence: dto.evidence ?? [],
      refundAmount: fromCents(totalRefundAmount), refundItemAmount: fromCents(itemRefundAmount),
      refundShippingAmount: fromCents(shippingRefundAmount), returnShippingFee: fromCents(returnShippingFee),
      returnShippingCost: fromCents(returnShippingCost),
      inspectionPassed: null, inspectionNote: null, inspectedAt: null,
      reviewedAt: null,
    });
    ...
    await this.publish(saved, order.orderNumber, ownerId, "return.requested", request.description);
    return saved;
  }
  ...
  async list(ownerId: string, orderId: string) {
    const order = await this.orders.findOwnedById(ownerId, orderId);
    if (!order) throw new NotFoundException("Không tìm thấy đơn hàng.");
    const requests = await this.repository.find({ where: { orderId, ownerId }, order: { createdAt: "DESC" } });
    return Promise.all(requests.map((request) => this.ensureReturnShippingQuote(request, order)));
  }
  ...
  async getForCustomer(ownerId: string, returnId: string) {
    const request = await this.repository.findOne({ where: { id: returnId, ownerId } });
    if (!request) throw new NotFoundException("Không tìm thấy yêu cầu hoàn hàng.");
  ...
  async cancel(ownerId: string, returnId: string) {
    ...
    if (request.status !== OrderReturnStatus.REQUESTED) throw new ConflictException("Yêu cầu đã được shop xử lý.");
    request.status = OrderReturnStatus.CUSTOMER_CANCELLED;
    const saved = await this.repository.save(request);
    const order = await this.orders.findOwnedById(ownerId, request.orderId);
    await this.publish(saved, order?.orderNumber ?? "", ownerId, "return.cancelled");
    return saved;
  }

  // Seller queue chỉ trả request thuộc shop được resolve từ session, đồng thời bổ sung quote cho request legacy.
  async listForSeller(currentUser: SellerOrderUserContext, status?: OrderReturnStatus) {
    const shopId = await this.sellerShopClient.getOwnedShopId(currentUser);
    const requests = await this.repository.find({ where: { shopId, ...(status ? { status } : {}) }, order: { createdAt: "ASC" } });
    return Promise.all(requests.map(async (request) => {
      const order = await this.orders.findOwnedById(request.ownerId, request.orderId);
  ...
  async review(currentUser: SellerOrderUserContext, returnId: string, dto: ReviewOrderReturnDto, status: OrderReturnStatus) {
    const shopId = await this.sellerShopClient.getOwnedShopId(currentUser);
    const request = await this.repository.findOne({ where: { id: returnId, shopId } });
    if (!request) throw new NotFoundException("Không tìm thấy yêu cầu hoàn hàng.");
    if (request.status !== OrderReturnStatus.REQUESTED) throw new ConflictException("Yêu cầu đã được xử lý.");
    const reviewNote = dto.note?.trim() || "";
    if (status === OrderReturnStatus.REJECTED && reviewNote.length < 10) {
      throw new BadRequestException("Seller phải nhập lý do từ chối ít nhất 10 ký tự.");
    }
    request.status = status === OrderReturnStatus.APPROVED ? OrderReturnStatus.AWAITING_SHIPMENT : OrderReturnStatus.REJECTED;
    ...
    await this.publish(saved, order?.orderNumber ?? "", request.ownerId, status === OrderReturnStatus.APPROVED ? "return.approved" : "return.rejected");
    return saved;
  }
  ...
  async markInTransit(returnId: string) {
    ...
    if (request.status !== OrderReturnStatus.AWAITING_SHIPMENT) {
      throw new ConflictException("Yêu cầu không ở trạng thái chờ tạo vận đơn hoàn.");
    }

    request.status = OrderReturnStatus.IN_TRANSIT;
    const saved = await this.repository.save(request);
    const order = await this.orders.findOwnedById(request.ownerId, request.orderId);
    await this.publish(saved, order?.orderNumber ?? "", request.ownerId, "return.in_transit");
    return saved;
  }
  ...
  async markReceived(returnId: string) {
    ...
    if (![OrderReturnStatus.AWAITING_SHIPMENT, OrderReturnStatus.IN_TRANSIT].includes(request.status)) throw new ConflictException("Trạng thái không thể nhận hàng hoàn.");
    request.status = OrderReturnStatus.RECEIVED;
    const saved = await this.repository.save(request);
    const order = await this.orders.findOwnedById(request.ownerId, request.orderId);
    await this.publish(saved, order?.orderNumber ?? "", request.ownerId, "return.received");
    return saved;
  }
  ...
  async getShippingContext(returnId: string) {
    const request = await this.repository.findOne({ where: { id: returnId } });
    if (!request) throw new NotFoundException("Không tìm thấy yêu cầu hoàn hàng.");
    if (![OrderReturnStatus.AWAITING_SHIPMENT, OrderReturnStatus.IN_TRANSIT].includes(request.status)) throw new ConflictException("Yêu cầu chưa sẵn sàng tạo vận đơn hoàn.");
    const order = await this.orders.findOwnedById(request.ownerId, request.orderId);
    if (!order) throw new NotFoundException("Không tìm thấy đơn hàng gốc.");
    ...
    return {
      ...
      items: (order.items ?? []).filter((item) => selected.has(item.id)).map((item) => ({
  ...
  async updateReturnShippingCost(returnId: string, amount: string) {
    const request = await this.repository.findOne({ where: { id: returnId } });
    if (!request) throw new NotFoundException("Không tìm thấy yêu cầu hoàn hàng.");
    if (![OrderReturnStatus.AWAITING_SHIPMENT, OrderReturnStatus.IN_TRANSIT].includes(request.status)) {
      throw new ConflictException("Yêu cầu chưa ở trạng thái tạo vận đơn hoàn.");
    }

    const actualCost = this.roundVnd(toCents(amount));
    const customerDeduction = this.isSellerFault(request.reason) ? 0n : actualCost;
    const itemAmount = this.roundVnd(toCents(request.refundItemAmount));
    const outboundShippingRefund = this.roundVnd(toCents(request.refundShippingAmount));
    ...
    request.refundAmount = fromCents(
      this.capRefundAmount(finalRefund, (await this.orders.findOwnedById(request.ownerId, request.orderId))?.totalAmount),
    );
    return this.repository.save(request);
  }

  // Seller ghi nhận pass/fail sau khi kiểm tra; phase hiện tại chỉ dừng ở chờ hoàn tiền hoặc chờ gửi trả sản phẩm.
  async inspect(currentUser: SellerOrderUserContext, returnId: string, dto: InspectOrderReturnDto) {
    const shopId = await this.sellerShopClient.getOwnedShopId(currentUser);
    const request = await this.repository.findOne({ where: { id: returnId, shopId } });
    if (!request) throw new NotFoundException("Không tìm thấy yêu cầu hoàn hàng.");
    if (request.status !== OrderReturnStatus.RECEIVED) throw new ConflictException("Hàng hoàn chưa được ghi nhận tại shop.");
    request.inspectionPassed = dto.passed;
    request.inspectionNote = dto.note?.trim() || null;
    request.inspectedAt = new Date();
    request.status = dto.passed ? OrderReturnStatus.REFUND_PENDING : OrderReturnStatus.INSPECTION_FAILED;
    const saved = await this.repository.save(request);
    const order = await this.orders.findOwnedById(request.ownerId, request.orderId);
    await this.publish(saved, order?.orderNumber ?? "", request.ownerId, dto.passed ? "return.inspection.passed" : "return.inspection.failed");
    return saved;
  }
  ...
  private validateEvidence(dto: CreateOrderReturnDto): void {
    ...
    if (images.length > 5 || videos.length > 1) throw new BadRequestException("Tối đa 5 ảnh và 1 video cho bằng chứng hoàn hàng.");
    if (EVIDENCE_REQUIRED_REASONS.has(dto.reason) && images.length === 0) throw new BadRequestException("Lý do này cần ít nhất một ảnh bằng chứng.");
  }
  ...
  private async quoteReturnShippingCost(
    ...
    if (!this.shippingClient) throw new BadRequestException("Shipping Service chưa sẵn sàng báo phí hoàn hàng.");
    const selected = new Set(itemIds);
    const items = (order.items ?? []).filter((item) => selected.has(item.id));
    if (!items.length) throw new BadRequestException("Không có sản phẩm để báo phí hoàn hàng.");
    const shippingAddress = order.shippingAddress;
    const quote = await this.shippingClient.calculateQuote({
      shopId,
      shipmentKind: "RETURN",
      to: {
        contactName: String(shippingAddress.fullName ?? shippingAddress.contactName ?? "Người nhận"),
        phone: String(shippingAddress.phone ?? ""),
        addressLine: String(shippingAddress.street ?? shippingAddress.addressLine ?? ""),
        ...
        ...(shippingAddress.ghnAddress
          ? { ghnAddress: shippingAddress.ghnAddress as GhnAddressSelectionInput }
          : {}),
      },
      weightGrams: Math.max(1, Math.round(items.reduce((sum, item) => sum + (item.packageWeightGrams ?? 1) * item.quantity, 0))),
      lengthCm: Math.max(1, Math.round(Math.max(...items.map((item) => Number(item.packageLengthCm ?? 1))))),
      widthCm: Math.max(1, Math.round(Math.max(...items.map((item) => Number(item.packageWidthCm ?? 1))))),
      heightCm: Math.max(1, Math.round(items.reduce((sum, item) => sum + Number(item.packageHeightCm ?? 1) * item.quantity, 0))),
      value: Number(itemAmount) / 100,
      codAmount: 0,
    });
    const quotedCost = this.roundVnd(toCents(quote.fee));
    if (quotedCost <= 0n) throw new BadRequestException("GHN không trả về phí hoàn hàng hợp lệ.");
    return quotedCost;
  }
  ...
  private async ensureReturnShippingQuote(
    ...
    if (toCents(request.returnShippingCost) > 0n || !quoteStatuses.has(request.status)) return request;
    const selectedItems = (order.items ?? []).filter((item) => request.itemIds.includes(item.id));
    const itemAmount = this.roundVnd(selectedItems.reduce((sum, item) => sum + toCents(item.lineTotal), 0n));
    const quotedCost = await this.quoteReturnShippingCost(order, request.itemIds, itemAmount, request.shopId);
    const customerDeduction = this.isSellerFault(request.reason) ? 0n : quotedCost;
    request.returnShippingCost = fromCents(quotedCost);
    request.returnShippingFee = fromCents(customerDeduction);
    request.refundAmount = fromCents(
      this.capRefundAmount(
        toCents(request.refundItemAmount) + toCents(request.refundShippingAmount) - customerDeduction,
        order.totalAmount,
      ),
    );
    return this.repository.save(request);
  ...
  private isSellerFault(reason: OrderReturnReason): boolean {
    return [OrderReturnReason.DAMAGED, OrderReturnReason.WRONG_ITEM, OrderReturnReason.MISSING_ITEM, OrderReturnReason.NOT_AS_DESCRIBED].includes(reason);
  }
  ...
  private calculateShippingRefund(
    order: {
      ...
      items?: Array<{ id: string; sellerShopId: string | null; lineTotal: string }>;
    },
    itemIds: string[],
    itemAmount: bigint,
    ...
    const shippingFee = breakdown?.fee !== undefined
      ? toCents(String(breakdown.fee))
      : toCents(order.shippingFee);
    if (subtotal <= 0n || shippingFee <= 0n) return 0n;

    const shopItems = (order.items ?? []).filter((item) => item.sellerShopId === shopId);
    const shopSubtotal = shopItems.reduce((sum, item) => sum + toCents(item.lineTotal), 0n);
    const allocationBase = breakdown?.fee !== undefined && shopSubtotal > 0n ? shopSubtotal : subtotal;
    const cappedItemAmount = itemAmount > allocationBase ? allocationBase : itemAmount;
    const proportionalAmount = (shippingFee * cappedItemAmount + allocationBase / 2n) / allocationBase;
  ...
  private capRefundAmount(value: bigint, originalOrderAmount?: string | null): bigint {
    ...
    return nonNegativeValue > maximumRefundable ? maximumRefundable : nonNegativeValue;
  }

  // Phát snapshot sau commit; request mới ưu tiên mô tả Customer, các bước sau dùng ghi chú xử lý gần nhất.
  private async publish(request: OrderReturnRequest, orderNumber: string, customerUserId: string, eventName: ReturnEventName, noteOverride?: string | null): Promise<void> {
    if (!orderNumber) return;
    await this.events.publishReturnChanged({
      eventName, returnId: request.id, orderId: request.orderId, orderNumber, shopId: request.shopId,
      customerUserId, sellerUserId: request.sellerUserId, status: request.status, refundAmount: request.refundAmount,
      reason: request.reason, note: noteOverride ?? request.reviewNote,
    });
  }
}

type ReturnEventName = "return.requested" | "return.approved" | "return.rejected" | "return.cancelled" | "return.in_transit" | "return.received" | "return.inspection.passed" | "return.inspection.failed";
